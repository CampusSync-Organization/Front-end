...
const Navbar = forwardRef(function Navbar(_, forwardedRef) {
  ...
  return (
    <nav
      ref={navRef}
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-out bg-white/80 backdrop-blur-xl border-b border-slate-200/60 shadow-sm ${isScrolled ? "py-1" : "py-2"}`}
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-12 md:h-14">
          <Link to="/" className="landing-nav-link flex items-center gap-2.5">
            ...
            />
            <span
              className={`hidden sm:inline text-[17px] font-semibold tracking-tight ${isLight ? "text-amber-500" : "text-amber-400"}`}
            >
              CampusSync
            </span>
          </Link>
          ...
          >
            {navLinks.map((link) => (
              <a
                ...
                className={`relative landing-nav-link text-[15px] font-medium px-4 py-2.5 rounded-full transition-colors duration-150 ${isLight ? "text-slate-900" : "text-white"}`}
              >
                <span className="relative z-10">{link.label}</span>
                <Motion.div
                  initial={{ scaleX: 0 }}
                  animate={{ scaleX: hoveredLink === link.label ? 1 : 0 }}
                  className="absolute -bottom-1 left-0 right-0 h-0.5 bg-amber-400 shadow-[0_0_12px_rgba(251,191,36,0.8)] z-0 origin-center"
          ...
          <div className="flex items-center gap-3 shrink-0">
            <Link
              to="/login"
              className="hidden sm:inline-block text-[15px] font-medium px-4 py-2.5 rounded-full text-slate-900"
            >
              Sign in
            </Link>
            <Link
              to="/signup"
              className="text-[15px] font-semibold px-5 py-2.5 rounded-full bg-amber-400 text-slate-900 transition-colors hover:bg-amber-300"
            >
              Sign up
            </Link>
            <button
        ...
        {isMobileOpen && (
          <div className="md:hidden -mx-4 mt-0 px-4 pt-4 pb-6 rounded-b-2xl border-t bg-white border-slate-200">
            <div className="flex flex-col gap-0.5">
              {navLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className="block text-[15px] font-medium py-3.5 px-4 rounded-xl text-slate-700"
            ...
            <div className="flex gap-3 pt-4 mt-2 border-t border-slate-200">
              <Link
                to="/login"
                className="flex-1 text-center text-[15px] font-medium py-3.5 rounded-xl border border-slate-200 text-slate-800"
                onClick={() => setIsMobileOpen(false)}
              >
                Sign in
              </Link>
              <Link
                to="/signup"
                className="flex-1 text-center text-[15px] font-semibold py-3.5 rounded-xl bg-amber-400 text-slate-900"
                onClick={() => setIsMobileOpen(false)}
              >
                Sign up
              </Link>
...
const Hero = forwardRef(function Hero(_, forwardedRef) {
  ...
  return (
    <section
      ref={sectionRef}
      id="home"
      className="relative w-full min-h-screen bg-white flex items-center overflow-hidden pt-24 pb-20"
    >
      ...
      <div className="landing-hero-card relative z-20 w-full max-w-7xl mx-auto px-6 sm:px-12 lg:px-20 mt-12 sm:mt-16 flex flex-col items-start text-left">
        <h1 className="landing-hero-heading text-[56px] sm:text-[72px] lg:text-[88px] xl:text-[96px] font-black leading-[1.02] tracking-tighter mb-10 max-w-[800px]">
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#000000] via-[#0a2a5a] to-[#0b3a7a]">
            Where Learning
          </span>{" "}
          <br />
        ...
        <div className="landing-hero-paragraph flex flex-col xl:flex-row xl:items-start gap-8 max-w-[900px] mb-14">
          ...
          <div className="xl:w-2/3">
            <p className="text-[17px] sm:text-[20px] text-slate-600 font-medium leading-[1.6]">
              <strong className="text-slate-800">We get it.</strong> Campus life
              can feel isolating. Finding your people shouldn't feel like an
              impossible mission.
              <br />
              <br />
              CampusSync connecting you with students who explicitly share your
              unique interests, values, and academic goals—making it effortless
              to build genuine friendships and form meaningful project teams.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 landing-hero-cta">
          <Link
            to="/signup"
            className="group relative inline-flex items-center justify-center bg-[#0f172a] text-white px-12 py-5 rounded-full text-[17px] font-bold tracking-wide transition-all shadow-[0_12px_24px_-8px_rgba(15,23,42,0.5)] hover:shadow-xl hover:-translate-y-1 hover:bg-slate-800 w-full sm:w-auto"
          >
            Join the Network
            <ArrowRight
...
const storyChapters = [
  {
    label: "The Problem",
    text: "As first-year college students, we faced a harsh reality—the campus felt enormous, and despite being surrounded by thousands of students, finding people who truly understood us seemed impossible. The traditional ways of meeting people weren't cutting it.",
  },
  {
    label: "The Struggle",
    text: "When project time came around, it was even worse. We either worked with classmates we barely knew or desperately hoped for compatible teammates. Many found themselves in groups with incompatible styles.",
  },
  {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            The passionate people behind CampusSync.
          </p>
        </div>
