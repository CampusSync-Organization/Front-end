...
function SignUpPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.from(".brand-panel", { x: -30, opacity: 0, duration: 0.6 })
        ...
        .from(".form-field", { y: 15, opacity: 0, stagger: 0.08, duration: 0.4 }, "-=0.2")
  ...
  return (
    <div ref={containerRef} className="min-h-screen flex bg-[#fbfbfd] overflow-hidden">
      ...
      <div className="brand-panel hidden lg:flex lg:w-1/2 bg-[#1d1d1f] relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 flex flex-col justify-between w-full px-12 py-10">
          ...
          <div className="brand-text">
            ...
            <ul className="mt-6 space-y-2">
              {features.map((feature, idx) => (
                <li key={idx} className="flex items-center gap-3 text-white/70 text-[14px]">
          ...
          <p className="text-[12px] text-white/30">© {new Date().getFullYear()} CampusSync</p>
        </div>
      </div>

      {/* Right: Form - same visual size as signup, no scroll */}
      <div className="flex-1 flex flex-col justify-center items-center px-4 sm:px-8 py-6 min-h-0 shrink-0">
        <div className="w-full max-w-[420px] sm:max-w-[480px]">
          <Link to="/" className="lg:hidden inline-flex items-center gap-2.5 mb-5">
          ...
          <div className="form-card">
            ...
            <form id="signup-form" onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="form-field space-y-1.5">
                  <label htmlFor="name" className="block text-[13px] font-medium text-[#1d1d1f]">
                    Name
                  </label>
              ...
              <button
                type="submit"
                className="form-button w-full h-11 rounded-xl bg-[#FCA311] text-[#1d1d1f] font-semibold text-[15px] transition-all hover:bg-[#E89310] active:scale-[0.99] flex items-center justify-center gap-2 mt-1 shadow-lg shadow-[#FCA311]/20"
              >
                Create account
            ...
            <div className="flex items-center gap-3 mt-3 my-3">
              ...
              <span className="text-[11px] text-[#86868b] uppercase tracking-wide">or</span>
              <div className="flex-1 h-px bg-[#d2d2d7]" />
            </div>
            ...
            <p className="mt-3 text-center text-[13px] text-[#86868b]">
              ...
              <Link to="/login" className="font-medium text-[#FCA311] hover:text-[#E89310] transition-colors">
                Sign in
              </Link>
            </p>
          </div>
