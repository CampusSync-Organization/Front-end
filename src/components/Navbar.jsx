    ...
    <nav className={navBarClass}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 md:h-16">
                        ...
                        <div className="flex flex-col gap-0.5 pt-1">
                            {appLinks.map((link) => (
                                <Link
                                    key={link.to}
                                    to={link.to}
                                    className={`block text-[15px] font-medium py-3 px-3 rounded-xl transition-colors ${location.pathname === link.to
                                        ? "text-primary bg-primary/10"
                                        : "text-slate-600 hover:text-slate-900 hover:bg-slate-100/80"
                                        }`}
          ...
          <button
            type="button"
            className="md:hidden flex h-10 w-10 items-center justify-center rounded-xl text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-all duration-200"
            onClick={() => setMobileOpen((o) => !o)}
            aria-label="Toggle menu"
          >
            {isMobileOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
