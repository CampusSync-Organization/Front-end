...
export default function Navbar() {
    ...
    const linkClass = (to) => {
        ...
        return `text-[14px] font-medium transition-colors duration-200 px-3 py-2 rounded-lg ${active
                ? "text-primary bg-primary/10"
                : "text-slate-600 hover:text-slate-900 hover:bg-slate-100/80"
            }`;
    };

    const navBarClass = `fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300 ease-out ${isScrolled
                        ...
                        {appLinks.map((link) => {
                            ...
                            return (
                                <Link
                                    ...
                                    className={`relative px-4 py-2 rounded-xl text-[14px] font-medium transition-colors duration-200 ${isActive ? "text-[#14213D]" : "text-slate-600 hover:text-slate-900"
                                        }`}
                                >
                            ...
                            {appLinks.map((link) => {
                                ...
                                return (
                                    <Link
                                        key={link.to}
                                        to={link.to}
                                        className={`block text-[15px] font-medium py-3.5 px-4 rounded-xl transition-colors ${active ? "text-[#14213D] bg-slate-100" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                                            }`}
